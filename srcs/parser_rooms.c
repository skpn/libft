...
int		parse_room(t_lem *lem, char **line, int *index)
{
	...
	start_end = 0;
	if (!(manage_start_end(lem, line, &start_end)) || !(room = new_room(0, 0)))
		return (0);
	i = -1;
	while ((*line)[++i] && (*line)[i] != ' ' && (*line)[i] != '\n')
		;
	if (!(room->name = ft_strsub(*line, 0, i)) || !(*line)[i]
		|| !parse_coord(line, &i))
		return (0);
	...
	lem->nb_rooms++;
	return ((lem->anthill = ft_strjoin_free(&(lem->anthill), line, BOTH)))
}
