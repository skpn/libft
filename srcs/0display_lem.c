...
/*   Updated: 2019/11/02 22:16:24 by sikpenou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_rooms_addr(t_lst *rooms)
{
	t_lst	*elem;
	t_room	*room;
...
void	print_basic(t_room *room, char *args, int opt)
{
	...
	sep = opt == 2 ? "\n" : ", ";
	ft_strchr_pos(args, 'n') > -1 ? printf("'%s' ", room->name) : 0;
	ft_strchr_pos(args, 'a') > -1 ? printf("%saddr %p ", sep, room) : 0;
	ft_strchr_pos(args, 'd') > -1 ? printf("%sdist: %u ", sep, room->dist) : 0;
	ft_strchr_pos(args, 'w') > -1 ? printf("%swalk: %u ", sep, room->walk) : 0;
	ft_strchr_pos(args, 'h') > -1 ? printf("%shas lvl: %u ", sep, room->has_lvl) : 0;
	opt ? printf("\n") : 0;
}
...
void	print_room(t_room *room, char *args, int opt)
{
	...
	if (ft_strchr_pos(args, 'p') > -1)
		print_family(room, PARENTS, "n");
	else if (ft_strchr_pos(args, 'c') > -1)
		print_family(room, CHILDREN, "n");
	else if (ft_strchr_pos(args, 'f') > -1)
		print_family(room, FAMILY, "n");
}

void	print_lvl(t_lst *lvl, unsigned dist)
{
	t_lst	*elem;
	t_room	*room;
...
void	print_rooms(t_lst *rooms, char *args, int opt)
{
	while (rooms)
	{
		print_room(rooms->content, args, opt);
		printf("\n");
		rooms = rooms->next;
	}
}
